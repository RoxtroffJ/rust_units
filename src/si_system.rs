...
//! instead of just `Solve<A, B> for D`, but then you would also need to do an implementation for [`Quantity`](crate::Quantity).
//!
//! - Implement the operator on exponents (`E` parameter of [`inners::SIDim`])
//! and generalize to [`SIDimension`]s using the [`si_impl_*_op`](crate::si_impl_bin_op) macro (advanced).
//! ```
...
//! assert_eq!(format!("{speed}"), "0.5 s^-1.m")
//! ```
//! As you can see, this technique is more complicated, but allows for more custom behavior when using custom exponent types.
...
///     (
///         /// Comment for Time (optional)
///         pub Time,
///         /// Comment for TimeID (optional)
///         pub(crate) TimeID
///         ; "s" // after a ; you can define symbol for this dimension for prints (optional, do not place the ; if nothing behind it).
...
/// =
///     /// Comment for MySILikeSystem (optional)
///     pub MySILikeSystem}
///
/// // Now we have access to the three dimensions. They were defined with their respective ID for differentiation.
...
macro_rules! si_add_dim {
    ...
    ($System:ty => $(,)? = $(#[$meta:meta])* $sys_vis:vis $NewSystem:ident ) => {
        $(#[$meta])*
        $sys_vis type $NewSystem = $System;
    };
    ...
    // Same as previous one, but with exponent omitted.
